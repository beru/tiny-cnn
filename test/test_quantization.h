...
TEST(quantization_utils, avoid_bias) {
  for (int i = 0; i < 256; ++i) {
    const float as_float =
      core::kernels::quantized_to_float<uint8_t>(static_cast<uint8_t>(i), 0.0f, 2.0f);
...
TEST(quantization_utils, quantize_down_and_shrink_range) {
  // For this test we have an input that has the theoretical range of -256.0f
  // to
  ...
  // the available range in the lower bit depth, and update to the true min
  // and
  // max ranges.
